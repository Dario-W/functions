...
#' 
#' Create a list for xa, xb, ya, yb with time blocks using NAs in the dataset as seperation points, then forward these lists to the pulls & anchors function by Ari.
#'
#' 
...
#' @param noise_thresh 
#' @param plot_results 
#' @param include_initial_fusion 
#' @param include_final_fission 
#'
#' @return
#' @export
#'
...
get_pulls_and_anchors_with_NA <- function(xa, xb, ya, yb, a, b, noise_thresh = 5, plot_results = F, include_initial_fusion = F, include_final_fission = F, NA_tolerance = 0, min_time = NULL, verbose = FALSE){
  ...
  if (anyNA(xa) | anyNA(xb)) { #check if there are any NAs in the vectors xa or xb. Assuming that the vector pairs xa and ya or xb and yb within possess NA in same positions.
    ...
    miss.xab <- sort(unique(c(which(is.na(xa)), which(is.na(xb))))) #gain all indexes of NAs in xa and xb
    
    breakpts <- c(0, miss.xab, length(xa)+1) #create index points as markers for where to split the vector into time blocks (start and stop points)
  ...
  else{get_pulls_and_anchors(xa=xa, xb=xb, ya=ya, yb=yb, a=a, b=b,
                             noise_thresh = noise_thresh, plot_results = plot_results,
                             include_initial_fusion = include_initial_fusion, include_final_fission = include_final_fission)
  }
}
